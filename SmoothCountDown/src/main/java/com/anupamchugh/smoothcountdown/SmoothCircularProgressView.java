...
/**
 * Created by anupamchugh on 21/02/17.
 */
...
public class SmoothCircularProgressView extends View {
    ...
    private SmoothProgressAnimationListener progressAnimationListener;

    public SmoothCircularProgressView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        setDefaultValues();
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SmoothCircularProgressView, 0, 0);

        try {
            mProgress = typedArray.getFloat(R.styleable.SmoothCircularProgressView_scpv_progress, mProgress);
            mCircleWidth = typedArray.getDimension(R.styleable.SmoothCircularProgressView_scpv_circle_width, mCircleWidth);
            backgroundStrokeWidth = typedArray.getDimension(R.styleable.SmoothCircularProgressView_scpv_background_circle_width, backgroundStrokeWidth);
            mCircleColor = typedArray.getInt(R.styleable.SmoothCircularProgressView_scpv_circle_color, mCircleColor);
            mBackgroundColor = typedArray.getInt(R.styleable.SmoothCircularProgressView_scpv_background_circle_color, mBackgroundColor);
            mTextColor = typedArray.getInt(R.styleable.SmoothCircularProgressView_scpv_text_color, mTextColor);
            mTextSize = typedArray.getInt(R.styleable.SmoothCircularProgressView_scpv_text_size, mTextSize);
            mTextPrefix = typedArray.getString(R.styleable.SmoothCircularProgressView_scpv_text_prefix);
            mTextSuffix = typedArray.getString(R.styleable.SmoothCircularProgressView_scpv_text_suffix);
        } finally {
            typedArray.recycle();
        }
    ...
    public void setProgressWithAnimation(final float progress, int duration) {
        ...
        objectAnimator.setInterpolator(mInterpolator != null ? mInterpolator : new LinearInterpolator());
    ...
    public SmoothProgressAnimationListener getProgressAnimationListener() {
        return progressAnimationListener;
    }

    public void addAnimationListener(SmoothProgressAnimationListener progressAnimationListener) {
